...
pivoted_penetration AS (
    SELECT
        state,
        MAX(CASE WHEN fiscal_year = 2022  THEN penetration_rate ELSE NULL END) AS penetration_rate_2022,
        MAX(CASE WHEN fiscal_year = 2023  THEN penetration_rate ELSE NULL END) AS penetration_rate_2023,
        MAX(CASE WHEN fiscal_year = 2024  THEN penetration_rate ELSE NULL END) AS penetration_rate_2024
    FROM
        penetration_rates
    GROUP BY
        state
